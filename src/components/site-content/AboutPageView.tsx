...
export function AboutPageView({ content }: { content: SiteContentV1["about"] }) {
  const ctaIndex = content.teamCtaText.indexOf(content.teamCta);
  const beforeCta = ctaIndex >= 0 ? content.teamCtaText.slice(0, ctaIndex) : "";
  const afterCta =
    ctaIndex >= 0
      ? content.teamCtaText.slice(ctaIndex + content.teamCta.length)
      : content.teamCtaText;
