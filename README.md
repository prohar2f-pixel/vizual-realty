...
- менять тексты публичных страниц через черновик и предпросмотр, включая
  показатели «О нас», вступление и режим работы в контактах, а также подписи и
  пояснения формы заявки;
...
Сохранённые до расширения схемы полные JSON-снимки версии 1 читаются через узкую
совместимость: только при одновременном отсутствии всех новых полей они
дополняются встроенными значениями. Частично обновлённый или содержащий лишние
ключи снимок остаётся ошибкой строгой проверки. Следующее сохранение записывает
уже полный снимок. Испорченная или истёкшая cookie удаляется отдельным обработчиком
с фиксированным переходом на `/admin/login`.
...
11 МиБ для `/api/admin/team-images`. Лимит 11 МиБ относится ко всему multipart
запросу с техническими заголовками; сам файл по-прежнему не может превышать
10 МиБ.
...
Для backup и preflight orchestrator разрешает только абсолютную пару `pg_dump` и
`psql` из фиксированных системных каталогов: `/usr/lib/postgresql/<версия>/bin`
или `/usr/bin` в Linux и `C:\Program Files\PostgreSQL\<версия>\bin` в Windows.
Проверяются real path, имя файла, обычный исполняемый файл, общий `bin` и
принадлежность доверенному корню; пользовательский каталог, текущая папка и
наследуемый `PATH` не участвуют в выборе. Из окружения libpq-процессов также
удаляются `PATH` и `PATHEXT`. Если доверенная пара отсутствует, upgrade
останавливается до чтения `.env`, создания backup и `resolve`.
