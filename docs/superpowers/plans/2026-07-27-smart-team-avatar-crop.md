...
**Architecture:** Загрузчик в `team-images.ts` будет кодировать входные изображения в квадратный WebP с Sharp `attention` crop. Служебный скрипт применит тот же преобразователь к уже сохранённым файлам, не меняя их ID и ссылки в контенте.
...
- Для нового кадра используется встроенный алгоритм Sharp `attention`.
